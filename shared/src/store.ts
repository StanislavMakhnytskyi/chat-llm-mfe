...
const mockMessages: Message[] = [
  ...
  },
  {
    id: 'message-2',
    chatId: 'chat-design-system',
    content: 'Yes. We can keep it docked on desktop and animated on small screens.',
    sender: 'assistant',
  ...
  },
  {
    id: 'message-4',
    chatId: 'chat-react-roadmap',
    content: 'React 19.2.x gives us the current stable APIs and the latest docs baseline.',
    sender: 'assistant',
    timestamp: new Date(now.getTime() - 1000 * 60 * 18)
  },
  {
  ...
  },
  {
    id: 'message-6',
    chatId: 'chat-mfe-setup',
    content: 'By sharing a single workspace package and registering it as a federation singleton.',
    sender: 'assistant',
    timestamp: new Date(now.getTime() - 1000 * 60 * 31)
  }
];
      ...
      chats: state.chats.map((chat) =>
        chat.id !== message.chatId
          ...
          : {
              ...
              unread:
                message.sender === 'assistant' && state.selectedChatId !== message.chatId
                  ? chat.unread + 1
                  : 0
            }
      )
  ...
  addChat: (title) =>
    set((state) => {
      ...
      return {
        ...
        sidebarOpen: false,
        messages: [
          createMessage(chat.id, 'Start the conversation with a clear prompt.', 'assistant'),
          ...state.messages
        ]
      };
    })
